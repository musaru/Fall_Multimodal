...
class EmbGCN(nn.Module):
    def __init__(self, dim_in, dim_out, adj,cheb_k, embed_dim):
        ...
        self.cheb_k = cheb_k
        self.sym_norm_Adj_matrix = torch.from_numpy(sym_norm_Adj(adj)).to(torch.float32).to(device)
    ...
    def forward(self, x, node_embeddings):
        ...
        # 静态邻接矩阵
        x_static = torch.einsum("nm,bmc->bmc",torch.softmax(self.sym_norm_Adj_matrix,dim=-1),x.to(device))
        x_static = self.linear(x_static.to(device))
