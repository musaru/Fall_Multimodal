...
from models.musa_model import Model, adjGraph
...
def train(model,dataloader,optimizer,loss_fn,device="cpu",logger=None,writer=None,logging_timing=10,epoch=None,scaler=torch.cuda.amp.GradScaler(),max_norm=100,top_k=[1]):
    accum_iter = 1
...
def test(model,dataloader,loss_fn,device,logger=None,logging_timing=10,epoch=None,top_k=[1]):
    ...
    _prediction = torch.argmax(prediction, dim=1).to('cpu').detach().numpy().copy()
    _labels = labels.to('cpu').detach().numpy().copy() if labels.dim()==1 else torch.argmax(labels,dim=1).to('cpu').detach().numpy().copy()
    logger.info(f"[[[[[Classification Report]]]]]\n{classification_report(_labels,_prediction,digits=5)}")
    return cal_top_k_accuracy(prediction,labels,top_k=top_k)
...
def run(config=None,dirname=None,logger=None):
    SEED = 42
    fix_seed(SEED)

    output_dir_name = "log_"+datetime.datetime.now().isoformat()
    OUTPUT_DIR_PATH = os.path.join("./outputs",dirname,output_dir_name)
    ...
    logger.info(f"******** start {output_dir_name} ********")
    ...
    test_acc = test(model,dataloaders["test"],loss_fn,device,logger)

    logger.info(f"******** end {output_dir_name} ********")

    return output_dir_name, val_acc, test_acc
...
if __name__=="__main__":
    config=None
    DIRNAME = "hyperparameter_tuning_URFALL"
    hyperparameter_tuning_result = defaultdict(list)
    
    os.makedirs(os.path.join("./outputs/",DIRNAME),exist_ok=True)
    logger = create_logger(output_dir=os.path.join("./outputs/",DIRNAME),name=__name__,filename="log.txt")

    grid_search_renge = {
        # "embed_dim":[8,16,32,64],
        "embed_dim":[16,32,64],
        "n_stage":[1,2,3,4],
        "act_type":["relu", "leakyrelu", "tanh", "gelu"],
        
    }

    product = [x for x in product(*grid_search_renge.values())]
    product_dict = [dict(zip(grid_search_renge.keys(), r)) for r in product]
    
    for config in product_dict:
        output_dir_name, val_acc, test_acc = run(config,dirname=DIRNAME,logger=logger)

        for hyperparam_name in config.keys():
            hyperparameter_tuning_result[hyperparam_name].append(config[hyperparam_name])
        hyperparameter_tuning_result["valid_acc_@1"].append(val_acc[0])
        hyperparameter_tuning_result["test_acc_@1"].append(test_acc[0])
        hyperparameter_tuning_result["output_dir_name"].append(output_dir_name)
        
        df = pd.DataFrame(hyperparameter_tuning_result)
        df.to_csv(f"./outputs/{DIRNAME}.csv",index=False)