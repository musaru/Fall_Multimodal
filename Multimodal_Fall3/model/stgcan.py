...
class STGCAN(nn.Module):
    ...
    def __init__(self, in_channels, graph_args, num_class=None,
        ...
        self.st_gcan_networks = nn.ModuleList((
            st_gcan(in_channels, 64, kernel_size, 1, residual=False, **kwargs0),
            
            st_gcan(64, 64, kernel_size, 1, **kwargs),
            st_gcan(64, 64, kernel_size, 1, **kwargs),
            # st_gcan(64, 64, kernel_size, 1, **kwargs),
            st_gcan(64, 128, kernel_size, 2, **kwargs),
            st_gcan(128, 128, kernel_size, 1, **kwargs),
            # st_gcan(128, 128, kernel_size, 1, **kwargs),
            st_gcan(128, 256, kernel_size, 2, **kwargs),
            st_gcan(256, 256, kernel_size, 1, **kwargs),
            # st_gcan(256, 256, kernel_size, 1, **kwargs)
        ))
        ...
        if edge_importance_weighting:
            self.edge_importance = nn.ParameterList([
                ...
                for i in self.st_gcan_networks
            ])
        else:
    ...
    def forward(self, skel, sensor):
        # data normalization.
        N, C, T, V = skel.size()
        x = skel.permute(0, 3, 1, 2).contiguous()  # (N, V, C, T)
        x = x.view(N, V * C, T)
        x = self.data_bn(x)
        x = x.view(N, V, C, T)
        ...
        # forward.
        for gcn, importance in zip(self.st_gcan_networks, self.edge_importance):
